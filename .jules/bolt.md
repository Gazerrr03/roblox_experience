...
## 2025-05-22 - [MonsterLogic Squared Distance & Component Math]
**Learning:** In Luau, calculating `Vector3.Magnitude` and creating temporary `Vector3` objects (via subtraction) in a hot loop (like scanning for players) is expensive due to metatable dispatch and GC pressure. Switching to squared distance comparison (`dx*dx + dy*dy + dz*dz`) and raw numeric component math provides a significant speedup.
**Action:** Use squared distance for range checks and perform math on raw X, Y, Z components in performance-critical AI loops. Always include a `sightRange < 0` guard when squaring ranges to preserve correct behavior.
