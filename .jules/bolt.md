...
**Learning:** High-frequency monster AI routines like `pickNearestTarget` and `stepToward` suffer from significant overhead due to `Vector3` metatable dispatch and repeated `math.sqrt` (via `.Magnitude`). Replacing these with squared distance comparisons and direct numeric coordinate math (hoisting X, Y, Z components) reduces both CPU cycles and heap allocations.
**Action:** Use squared distance for proximity checks and perform component-wise arithmetic before a single final `Vector3` allocation in hot path motion logic.
