...
**Learning:** High-frequency monster AI logic (targeting and movement) suffers from Vector3 metatable overhead and math.sqrt calls. In Luau, performing distance comparisons using squared distances and raw numeric components (X, Y, Z) significantly reduces object churn and CPU time. Caching globals and utilizing a fast-path for target arrival further optimizes these hot paths.
**Action:** Use squared distance comparisons and raw coordinate math for all high-frequency spatial logic. Implement arrival fast-paths to skip unit vector and magnitude calculations when the target is within the move step.
