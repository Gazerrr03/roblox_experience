...
## 2026-05-22 - [Vector3 Math & Allocation Optimization]
**Learning:** In Luau's high-frequency loops, performing math on unpacked numeric components (X, Y, Z) and allocating a single result Vector3 is significantly faster than relying on vector metatable arithmetic. Additionally, using squared distance for range checks and overshooting detection avoids redundant `math.sqrt` calls, providing a measurable performance win in AI pathing.
**Action:** Prefer raw component math and squared distance comparisons in performance-critical AI and navigation modules like `MonsterLogic.luau`.
