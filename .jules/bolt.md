...
## 2025-05-22 - [Monster Logic AI Optimization]
**Learning:** High-frequency AI loops like `pickNearestTarget` and `stepToward` are major sources of `Vector3` allocation churn and metatable dispatch overhead. Using raw numeric component math (X, Y, Z) and squared distance comparisons can yield >5x performance gains compared to standard `Vector3` methods.
**Action:** Prefer raw component math and squared distance in all high-frequency AI or spatial query loops to minimize GC pressure and execution time.
