...
## 2026-04-12 - [Monster AI Vector Math Optimization]
**Learning:** High-frequency Vector3 math in Luau AI loops (like `pickNearestTarget` and `stepToward`) is heavily bottlenecked by metatable dispatch and `math.sqrt` calls. Using squared distance comparisons and raw numeric component math (X, Y, Z) to avoid metatable overhead can yield a ~5x-6x speedup. A `sightRange < 0` guard is essential when using squared distances to maintain parity with original comparison logic.
**Action:** In high-frequency AI or collision loops, unpack Vector3 components once and perform numeric math. Use squared distances for range checks to skip `math.sqrt`.
